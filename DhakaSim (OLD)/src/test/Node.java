...
public class Node {
    ...
    Node(int index, int nodId, double x, double y,double z) {
        this.index = index;
        this.id = nodId;
        this.x = x;
        this.y = y;
    ...
    void adaptiveSignalChange(int simulationTime) {
        System.out.print("");
        if (simulationTime - timePassed >= min(DhakaSimFrame.SIGNAL_CHANGE_DURATION + pressureOnActiveBundle, 120)) {
    ...
    void manualSignaling() {
        if (isNodeClear()) {
            ...
            timePassed = DhakaSimFrame.simulationStep;
        }
    }
...
//            System.out.println("Sim step: " + simulationTime);
    ...
    public ArrayList<Passenger> releasePassengers(int capacity){
        if(capacity == 0 || capacity > passengerList.size()){
            return null;
        }
        ArrayList<Passenger> list = new ArrayList<>();
        Random random = new Random();
        int boundary = passengerList.size();
        for(int i = 0 ; i < capacity ; i++){
            int ind = random.nextInt(boundary--);
            list.add(passengerList.remove(ind));
        }
        return list;
    }

...
import static test.Utilities.getCarWidth;
...
public class Vehicle {
    ...
    private final static double L = 2; // speed exponent of GHR/F Motors DLC model
    ...
    private int passengersCount ; // new added ---> number of passengers in the vehicle
    ...
    private Point3D segmentStartPoint3D;  //newly added
    private Point3D segmentEndPoint3D;  //newly added
    ...
    ArrayList<Passenger> passengerList;//newly added to trace the passengers available in this node
    ...
    private int setCapacity(int type){
        if(type == 0){
            return 1;
        }
        else if(type == 1){
    ...
    public Vehicle(int vehicleId, int startTime, int type, double speed,int passengersCount, Color color, int demandIndex, int pathIndex, int linkIndexOnPath, double segStartX, double segStartY,double segStartZ, double segEndX, double segEndY,double segEndZ, boolean reverseLink, boolean reverseSegment, Link link, int segmentIndex, int stripIndex) {
        ...
        //newly added
        this.passengersCount = passengersCount ; // number of passengers in the vehicle
        this.passengerList = new ArrayList<>();
        this.passengerCapacity = setCapacity(type);
        //
        ...
        segmentStartPoint3D = new Point3D(0, 0,0);
        segmentEndPoint3D = new Point3D(0, 0,0);
        this.segmentStartPoint3D.x = segStartX;
        this.segmentStartPoint3D.y = segStartY;
        this.segmentStartPoint3D.z = segStartZ;
        this.segmentEndPoint3D.x = segEndX;
        this.segmentEndPoint3D.y = segEndY;
        this.segmentEndPoint3D.z = segEndZ;
    ...
    public void handlePassengers(Node node, int simulationTime){
        //we do here two things
        // for each passenger,
        // we either leave them or keep them
        // if we leave them we call finishJourney method, else we call addNodeToPathWhileTravelling method
        // then we take new passengers, and call startJourney method
        ArrayList<Passenger> passengersToRemove = new ArrayList<>();
        int indices = 0;
        for(Passenger passenger : passengerList){
            int choice = random.nextInt(2);
            if(choice == 1){//means he has to leave :3
                passenger.finishJourney(node.getId(), simulationTime);
                node.addPassenger(passenger);
                passengersToRemove.add(passenger);
            }
            else{
                passenger.addNodeToPathWhileTravelling(node.getId(), simulationTime);
            }
        }
        for(Passenger p : passengersToRemove){
            passengerList.remove(p);
        }
        boardPassengers(node, simulationTime);
        passengersCount = passengerList.size();
    }

    private boolean boardPassengers(Node node, int simulationTime){//private helper method to take passengers on board
        int capacity = passengerCapacity - passengersCount;
        int numBoards = random.nextInt(capacity);//should be greater than zero
        ArrayList<Passenger> passengers = node.releasePassengers(numBoards);
        if(passengers == null){
            return false;
        }
        while(!passengers.isEmpty()){
            Passenger passenger = passengers.remove(0);
            passenger.startJourney(node.getId(), this.type, simulationTime);
            passengerList.add(passenger);
        }
        return true;
    }

    public void dropAllPassengers(Node node, int simulationTime){ //called when a vehicle is removed
        for(Passenger passenger : passengerList){
            passenger.finishJourney(node.getId(), simulationTime);
            node.addPassenger(passenger);
        }
    ...
    public int getPassengersCount() {  //newly added
        return passengersCount;
    }

    public void setPassengersCount(int passengersCount) {  //newly added
...
//        this.leader = leader;
    ...
    private boolean controlSpeedInSegment() {
        ...
        //newly added
        if(vehicleId == 1){
            System.out.println(segmentStartPoint3D.x + " , "+ segmentStartPoint3D.y+ " , "+ segmentStartPoint3D.z);

            System.out.println(".........");
        }
        double cos_theta = 1.0 ;
        if (segmentStartPoint3D.z != segmentEndPoint3D.z){
            double diffrenceX = segmentStartPoint3D.x - segmentEndPoint3D.x ;
            double diffrenceY = segmentStartPoint3D.y - segmentEndPoint3D.y ;
            double diffrenceZ = segmentStartPoint3D.z - segmentEndPoint3D.z ;
            double hypotenuse = sqrt(diffrenceX*diffrenceX + diffrenceY*diffrenceY + diffrenceZ*diffrenceZ);
            double adjacent = sqrt(diffrenceX*diffrenceX + diffrenceY*diffrenceY );

            cos_theta = adjacent / (hypotenuse*10 );   //cos = bumi/otivuj
            //System.out.println("_________________"+cos_theta+"_________________________");
        }
        speed = getNewSpeed()*cos_theta;
...
//        System.out.printf("Vehicle: %d stuck in node: %d at time %d %n", vehicleId, node.getId(), simulationStep);
    ...
    {
        return  ((segmentLeaveTime - segmentEnterTime) * Constants.TIME_STEP);
    }//Riya

    private double getAvgSpeedInSegment() {     // CSE18
     ...
     */
    void calculateStatisticsAtEnd() { // newly added
        endTime = simulationStep;
        ...
        //newly added
//        System.out.println("passengers => "+passengersCount);
    }
    ...
    public double getEnterTime() {
        return segmentEnterTime*Constants.TIME_STEP;
    }

    //CSE18
    public double getLeaveTime() {
        return segmentLeaveTime*Constants.TIME_STEP;
    }
